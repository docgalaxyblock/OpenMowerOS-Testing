...
11. If you have `mower_config` and `openmower_version` configured you can now start openmower by executing `sudo systemctl start openmower`
...
Use `sudo podman exec -it open-mower /openmower_entrypoint.sh /bin/bash` to access the ROS enviroment and then use your command (e.g. `rostopic echo`)
